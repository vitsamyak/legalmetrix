...
export const AnalyticsPage = () => {
  ...
  const calculateMetrics = () => {
    ...
    const totalScore = inspections.reduce((acc, curr) => acc + (curr.compliance_score || 0), 0);
    const averageScore = Math.round(totalScore / inspections.length);
  ...
  return (
    <div className="space-y-6">
      <div className="mb-8">
        <h1 className="text-2xl font-bold font-heading text-content tracking-tight">Compliance Analytics</h1>
          ...
          <CardContent className="p-6">
            <div className="flex justify-between items-start">
              <div>
                ...
                <h3 className="text-3xl font-bold text-content mt-2">{loading ? '-' : \`\${complianceRate}%\`}</h3>
              </div>
          ...
          <CardContent className="p-6">
            <div className="flex justify-between items-start">
              <div>
                ...
                <h3 className="text-3xl font-bold text-content mt-2">{loading ? '-' : totalViolations}</h3>
              </div>
          ...
          <CardContent className="p-6">
            <div className="flex justify-between items-start">
              <div>
                ...
                <h3 className="text-3xl font-bold text-content mt-2">{loading ? '-' : \`\${averageScore}/100\`}</h3>
              </div>
      ...
      <Card>
        ...
        <CardContent>
          <div className="h-[400px] w-full">
            {loading ? (
              <div className="w-full h-full flex items-center justify-center text-content-muted">Loading chart data...</div>
            ) : trendData.length === 0 ? (
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={trendData} margin={{ top: 20, right: 30, left: 0, bottom: 0 }}>
                  ...
                  <Tooltip 
                    ...
                    contentStyle={{ backgroundColor: '#182238', borderColor: 'rgba(255,255,255,0.08)', borderRadius: '12px', color: '#F8FAFC' }}
                  />
