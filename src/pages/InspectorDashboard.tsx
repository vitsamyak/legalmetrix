                ...
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    {loading ? (
                      ...
                      <h3 className="text-3xl font-bold text-content mt-2">{stat.value}</h3>
                    )}
                  </div>
          ...
          <Card className="h-full">
            ...
            <CardContent className="flex flex-col items-center justify-center">
              <div className="h-[220px] w-full relative">
                {loading ? (
                  <SkeletonLoader className="w-full h-full rounded-full bg-white/5" />
                ) : (
                  <>
                    <ResponsiveContainer width="100%" height="100%">
                      <PieChart>
                        ...
                        <Tooltip 
                          contentStyle={{ backgroundColor: '#090C15', borderColor: 'rgba(255,255,255,0.1)', borderRadius: '12px', boxShadow: '0 10px 25px -5px rgba(0, 0, 0, 0.5)' }}
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}>
        <Card>
                ...
                {loading ? (
                  ...
                  recentInspections.map((item) => (
                    <tr key={item.id} className="hover:bg-surface-light/50 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <span className={`font-bold ${getScoreColor(item.compliance_score)}`}>{item.compliance_score !== null && item.compliance_score !== undefined ? item.compliance_score + '/100' : '--/100'}</span>
                      </td>
                      <td className="px-6 py-4">
          ...
          <div className="p-4 border-t border-border flex justify-center">
            <Link to="/inspections" className="text-sm font-medium text-primary hover:text-primary-dark transition-colors">
